...
一方、専用LINEアカウントのログインはまだ人間操作を行っていない。そのため、Windows/Androidの同一アカウント運用、ログイン耐性、A/B同期、RAW取得方式は未確認である。
...
| 専用LINEアカウントのPCログイン | 未確認 | QR、生体認証、パスワード入力を行っていない。 |
...
| Android版LINEのメイン端末利用 | 未確認 | LINEパッケージは存在するが、専用アカウントのログイン状態は確認していない。 |
